<template>
  <section class="max-w-5xl mx-auto my-12 px-4">
    ...
    <NuxtLink class="inline-block m-1 px-5 py-3 bg-neutral-100 hover:bg-neutral-200 w-32 text-center" to="/2048/step10"> Vorige</NuxtLink>
    <NuxtLink class="inline-block m-1 px-5 py-3 bg-neutral-100 hover:bg-neutral-200 w-32 text-center" to="/2048/step12"> Volgende</NuxtLink>
  </section>
</template>
  ...
  setup() {
    ...
    const checkKey = (event) => {
      ...
      if (event.key === 'ArrowUp') {
        moveUp(event, blocks.value, numCol.value)
        setTimeout(addBlock, 200)
      } else if (event.key === 'ArrowDown') {
        moveDown(event, blocks.value, numCol.value)
        setTimeout(addBlock, 200)
      } else if (event.key === 'ArrowLeft') {
        moveLeft(event, blocks.value, numCol.value)
        setTimeout(addBlock, 200)
      } else if (event.key === 'ArrowRight') {
        moveRight(event, blocks.value, numCol.value)
        setTimeout(addBlock, 200)
      }
    }
