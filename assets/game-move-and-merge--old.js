      ...
      if (gridValues.value[i][j] !== 0) {
        for (let k = -1; k > -gridValues.value.length; k--) {
          if (i - k > gridValues.value.length - 1) {
            ...
            break
          } else if (gridValues.value[i - k][j] === gridValues.value[i][j]) {
            // New position has same value as old position --> merge
            console.log('merge')
            gridValues.value[i - k][j] = gridValues.value[i - k][j] + gridValues.value[i][j]
            gridValues.value[i][j] = 0
            const blockIndexCurrentPosition = blocks.value.findIndex((element) => element.xPos === i && element.yPos === j)
            const blockIndexNewPosition = blocks.value.findIndex((element) => element.xPos === i - k && element.yPos === j)
            blocks.value[blockIndexCurrentPosition].xPos = i - k
            // Remove one of the blocks from array after merge
            blocks.value.splice(blockIndexNewPosition, 1)
            break
      ...
      if (gridValues.value[i][j] !== 0) {
        for (let k = -1; k > -gridValues.value.length; k--) {
          if (j - k > gridValues.value.length - 1) {
            ...
            break
          } else if (gridValues.value[i][j - k] === gridValues.value[i][j]) {
            // New position has same value as old position --> merge
            console.log('merge')
            gridValues.value[i][j - k] = gridValues.value[i][j - k] + gridValues.value[i][j]
            gridValues.value[i][j] = 0
            const blockIndexCurrentPosition = blocks.value.findIndex((element) => element.xPos === i && element.yPos === j)
            const blockIndexNewPosition = blocks.value.findIndex((element) => element.xPos === i && element.yPos === j - k)
            blocks.value[blockIndexCurrentPosition].yPos = j - k
            // Remove one of the blocks from array after merge
            blocks.value.splice(blockIndexNewPosition, 1)
            break
